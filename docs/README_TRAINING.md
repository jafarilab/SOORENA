...
✅ Files exist (generated by `prepare_data.py`):
...
- **Batch Size:** 16
- **Epochs:** configurable via `config.STAGE1_EPOCHS` (default: 3)

**Step 4: Validation & Checkpointing**

Monitors validation F1 score:
- Saves best model checkpoint
- Best checkpoint is written to `models/stage1_best.pt`
...
- **Optimizer:** AdamW (learning rate: 2e-5)
- **Batch Size:** 16
- **Epochs:** configurable via `config.STAGE2_EPOCHS` (default: 4)

**7 Mechanism Types (Stage 2 labels):**
...
- Saves best model checkpoint (selected by validation **macro F1**)
- Handles class imbalance with weighted loss (`sklearn` class weights)
...
Stage 2 Multiclass Classification:
  ...
  Per-class Performance:
    autophosphorylation: F1 = 0.85
    autoregulation:      F1 = 0.82
    autocatalytic:       F1 = 0.76
    ...
```
...
2. **Run predictions on unseen data:**
   ...
   For the full end-to-end workflow, see `docs/README.md`.
