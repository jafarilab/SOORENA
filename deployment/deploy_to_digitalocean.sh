...
# SOORENA Deployment Script for Digital Ocean
# This script automates the deployment of your Shiny app to Digital Ocean

set -e  # Exit on any error

echo "======================================================================"
echo "SOORENA Deployment to Digital Ocean"
echo "======================================================================"
echo ""

# Configuration
read -p "Enter your Digital Ocean Droplet Public IP: " SERVER_IP
...
echo "======================================================================"
ssh -i "$SSH_KEY" -o ConnectTimeout=10 root@$SERVER_IP "echo 'SSH connection successful!'"
...
mkdir -p /srv/shiny-server/soorena/data
mkdir -p /srv/shiny-server/soorena/www
EOF
...
scp -i "$SSH_KEY" "$APP_PATH/app.R" root@$SERVER_IP:/srv/shiny-server/soorena/
...
rsync -avz --progress -e "ssh -i $SSH_KEY" \
    "$DB_PATH" \
    root@$SERVER_IP:/srv/shiny-server/soorena/data/
...
rsync -avz --progress -e "ssh -i $SSH_KEY" \
    "$APP_PATH/www/" \
    root@$SERVER_IP:/srv/shiny-server/soorena/www/
...
echo "======================================================================"
ssh -i "$SSH_KEY" root@$SERVER_IP << 'EOF'
chown -R shiny:shiny /srv/shiny-server/soorena
chmod -R 755 /srv/shiny-server/soorena
EOF
...
echo "  3. Check logs if needed: ssh -i $SSH_KEY root@$SERVER_IP 'tail -f /var/log/shiny-server.log'"
echo ""
echo "======================================================================"
